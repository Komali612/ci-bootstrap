...
def render_workflow(cookbook: Cookbook, snapshot: RepoSnapshot) -> str:
    ...
    workflow = {
        "name": "CI",
        "on": {
            # Skip CI when only CI/CD config or docs change. CD provisioning writes
            # .github/ (notify workflow) and .harness/ (Harness pipeline) files, so
            # "running CD" shouldn't kick off a fresh CI build.
            "push": {
                "branches": [snapshot.default_branch],
                "paths-ignore": [".github/**", ".harness/**", "**.md"],
            },
            "pull_request": {"paths-ignore": [".github/**", ".harness/**", "**.md"]},
...
def dump_workflow(
    workflow: dict,
    header: str = "# Generated by cicd-bootstrap. Review before merging.",
) -> str:
    """Render a workflow dict to YAML with our conventions (key order kept,
    multi-line strings as block scalars). Shared by the CI generator and the CD
    (deploy) generator so both emit identically-formatted workflows."""
    text = yaml.dump(
